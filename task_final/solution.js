...
function sendRequest(name, phone, address, goods, sum) {
    let dataForReturn = {data: {client: {}, order: {}, goods: []}};
    //let currentGood = {title,count};
    let countOfGoods = goods.length;

    for (let i = 0; i < countOfGoods; i++ ) {
        dataForReturn.data.goods.push({"title":goods[i].title,"count":goods[i].count} );
    }

    //data.data.order.address = address;
    dataForReturn.data.order.address = "ул. " + address.street + ", дом " + address.house + ", " + address.entrance + " подъезд, " + address.floor + " этаж, кв " + address.flat;
    
    //data.order.sum = name + phone + address + goods + sum;
    dataForReturn.data.order.sum=sum;

    //data.client = 'Иван';
    dataForReturn.data.client = name + ' ' + phone;
    let jsonData = JSON.stringify(dataForReturn);
